...
def agregar_palabra_sentimiento(tabla_sentimientos, afd):
    ...
    # Agregar al AFD
    afd.agregar_transicion(afd.estado_inicial, palabra, f"q_{puntaje > 0 and 'POSITIVO' or 'NEGATIVO'}")
    
    # Guardar en el archivo para persistencia
    with open(os.path.join("data","palabras_y_puntajes.txt"), "a", encoding="utf-8") as f:
...
def eliminar_palabra_sentimiento(tabla_sentimientos):
    ...
    with open(os.path.join("data","palabras_y_puntajes.txt"), "r", encoding="utf-8") as f:
        lineas = f.readlines()
    
    with open(os.path.join("data","palabras_y_puntajes.txt"), "w", encoding="utf-8") as f:
        for linea in lineas:
